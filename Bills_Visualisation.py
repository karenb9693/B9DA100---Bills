...
p1 = bills.groupby("Company").Value.mean().sort_values(ascending=False)[:5].plot.bar(color=my_colors).set_title("mean value of bills for each company")
p2 = bills['Company'].value_counts().plot.barh(color=my_colors).set_title("Count of bills per Company")

p3 = bills.groupby(["Company","Type"]).Value.sum().sort_values(ascending=False)[:6].plot.bar(color=my_colors).set_title("sum value of bills for each company/type")
